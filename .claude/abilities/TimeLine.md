...
- **使用次数(短期)**: 0
- **使用次数(长期)**: 0
- **详情**: [[`.claude/timelines/{YYYY-MM-DD}/TIMELINE/[会话主题A].md`]]
...
- **使用次数(短期)**: 0
- **使用次数(长期)**: 0
- **详情**: [[`.claude/timelines/{YYYY-MM-DD}/TIMELINE/[会话主题B].md`]]
```
...
### 关联记忆时间线
- **[关联说明]**: [[`.claude/timelines/{YYYY-MM-DD}/TIMELINE/{会话主题}.md`|reference]]
```
...
### 存档管理
- 路径基准：当前项目根目录下的 `.claude/timelines/`，禁止使用全局路径 `~/.claude/timelines/`
- 会话文件存储路径：`.claude/timelines/{YYYY-MM-DD}/TIMELINE/`
- 会话文件命名格式：`{会话主题}.md`
- 索引文件路径：`.claude/timelines/{YYYY-MM-DD}/INDEX.md`
...
## 注意事项
- **强制路径约束**：所有时间线的读取和存储路径必须是当前项目根目录下的 `.claude/timelines/`，禁止使用全局路径 `~/.claude/timelines/`。当前项目根目录即为 Agent 启动时的工作目录
- 存档内容必须基于实际对话或真实历史数据 不得编造
- `.claude/timelines/{YYYY-MM-DD}/` 中的 `YYYY-MM-DD` 强制使用 Bash 查看日期命令获取
